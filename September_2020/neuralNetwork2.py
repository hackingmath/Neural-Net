...
class NeuralNetwork(object):
    ...
    def __init__(self, inputnodes, hiddennodes, hidden2nodes, outputnodes, learningrate):
        ...
        self.whh2 = np.random.normal(0.0, pow(self.h2nodes, -0.5),
                                       (self.h2nodes, self.hnodes))
        #from hidden2 layer to output
        self.who = np.random.normal(0.0, pow(self.onodes, -0.5),
                                       (self.onodes, self.h2nodes))
    ...
    def replicate(self):
        output = NeuralNetwork(self.inodes, self.hnodes, self.h2nodes, self.onodes, self.lr)
        output.wih = np.copy(self.wih)
        output.whh2 = np.copy(self.whh2)
    ...
    def mutate(self, rate=0.3, amount=0.1):
        ...
        flat_whh2 = self.whh2.flatten()
        for i, n in enumerate(flat_whh2):
            if random.random() < rate:
                flat_whh2[i] *= 1.0 + np.random.normal(0.0, amount)
        self.whh2 = np.copy(flat_whh2.reshape(self.h2nodes, self.hnodes))

        flat_who = self.who.flatten()
        for i, n in enumerate(flat_who):
            if random.random() < rate:
                flat_who[i] *= 1.0 + np.random.normal(0.0, amount)
        self.who = np.copy(flat_who.reshape(self.onodes, self.h2nodes))
        return self
    ...
    def train(self, inputs_list, targets_list):
        ...
        #calculate the signals emerging from hidden layer
        hidden2_outputs = self.activation_function(hidden2_inputs, False)
        ...
        # update the weights for the links between the
        # hidden and hidden2 layers
        self.whh2 += self.lr * np.dot((hidden2_errors * hidden2_outputs * \
                                      (1.0 - final_outputs)),
    ...
    def query(self, inputs_list):
        ...
        # calculate signals into final hidden layer
        hidden2_inputs = np.dot(self.whh2, hidden_outputs)

        # calculate the signals emerging from the
        # hidden layer2
        # print("Hidden2 inputs:", hidden2_inputs)
        hidden2_outputs = self.activation_function(hidden2_inputs, False)
